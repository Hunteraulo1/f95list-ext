...
<div class={variant === 'webapp' ? 'w-full h-full relative' : 'fixed w-wmain h-hmain top-0 left-0 z-20'}>
    ...
    {#if game}
      ...
      <Lazy height="33vh" fadeOption={{ delay: 0, duration: 0 }} keep={true} class="relative overflow-hidden bg-primary-foreground max-h-1/3" placeholder>
        <img
          alt={game.name}
          src={image ?? noImage}
          class="h-full w-full object-cover overflow-hidden bg-primary-foreground max-h-1/3"
