...
<div
  class={variant === "webapp"
    ? "overflow-hidden"
    : "fixed w-wmain h-hmain top-0 left-0 z-20"}
>
    ...
    {#if game}
      ...
      <div class="p-2 flex flex-col gap-4">
        ...
        <h1 class="mb-2 flex items-center gap-1 flex-wrap">
          <Badge
            style={typeColor(game.type)}
            class="text-primary-foreground font-bold"
          >
            {game.type}
          </Badge>
          <Badge
            style={statusColor(game.status)}
            class="text-primary-foreground font-bold"
          >
            {game.status}
          </Badge>
        ...
        {#if game.tname === "Traduction (mod inclus)"}
          <Alert
            description="Un mod peut-être nécessaire au bon fonctionnement de cette
              traduction. Veuillez lire les instructions du traducteur."
          />
        {:else if game.tname === "Pas de traduction"}
          <Alert
            description="Cette traduction à disparue. Veuillez nous contacter si vous en
